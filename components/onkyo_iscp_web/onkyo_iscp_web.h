...
class OnkyoIscpWeb : public Component, public onkyo_iscp::OnkyoIscpStateListener {
 ...
 protected:
  ...
  void handle_volume_up_(int type);
  void handle_volume_down_(int type);
  void handle_input_(Control *sender, int type);
  void handle_listening_mode_(Control *sender, int type);
  ...
  uint16_t main_tab_id_{0};
  uint16_t receiver_status_id_{0};
  uint16_t power_control_id_{0};
  uint16_t mute_control_id_{0};
  uint16_t volume_label_id_{0};
  uint16_t volume_control_id_{0};
  uint16_t volume_up_id_{0};
  uint16_t volume_down_id_{0};
  uint16_t input_control_id_{0};
  uint16_t listening_mode_control_id_{0};
  ...
  std::string listening_mode_state_;

  bool receiver_update_pending_{false};
  bool power_update_pending_{false};
  bool mute_update_pending_{false};
  bool volume_update_pending_{false};
  bool input_update_pending_{false};
  bool listening_mode_update_pending_{false};
